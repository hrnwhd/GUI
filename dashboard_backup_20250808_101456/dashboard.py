...
class OptimizedDashboardManager:
    ...
    def get_trade_log(self):
        ...
        # Get historical trades from file
...
# ===== OPTIMIZED WEBHOOK ENDPOINTS =====

@app.route('/webhook/live_data', methods=['POST'])
def webhook_live_data():
    """Webhook to receive live data from bot - now just saves for backup"""
    try:
        data = request.json
        # Save as backup, but main data comes from MT5 now
...
def webhook_account_update():
    ...
    try:
        data = request.json
        # This is now handled by background updater, but keep for compatibility
        return jsonify({"status": "success"})
    except Exception as e:
